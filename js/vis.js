...
function getUrlVars() { // 获取网页 URL 后的参数 返回： {perma: "E_PacBio_SKBR3_TRA_chr16_to_chr19_(5MB)"}
	var vars = {};
...
function draw_chunk_ref() {
	...
	// Whole reference chromosomes for the relevant references:   (顶部染色体)
	...
	ref_blocks.append("rect").attr("class","ref_block") // 染色体 rect
		.attr("x", 0)
		.attr("y", 0)
	...
	ref_blocks.append("text").attr("class","ref_block") // 染色体 text
		.filter(function(d) {return _Refs_show_or_hide[d.chrom]})
...
function comma_format(x) { // 数字格式化 eg: 30,361,388
	return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}

function bp_format(x) { // 1MB=1024KB, bp: 碱基对
...
function draw_chunk_ref_intervals() {
	...
	_scales.chunk_ref_interval_scale.range([_positions.multiread.ref_intervals.x, _positions.multiread.ref_intervals.x+_positions.multiread.ref_intervals.width]);  // 染色体 X 轴的 domain


	// 初始化 鼠标点击 reads 时的辅助线
	// _svg2.append("path").attr("id", "guide_line").style("stroke-dasharray", 7).style("stroke-opacity", 0.5).style("stroke-width", 1).attr("stroke", "red").style("visibility", "hidden")
	_svg2.append("rect").attr("id", "guide_line").attr("fill", "#fff").style("visibility", "hidden").attr("stroke","#ff0000")


	// Zoom into reference intervals where the read maps:
	_svg2.selectAll("rect.ref_interval").data(_Chunk_ref_intervals).enter()  // 上面的左右两个 rect
	...
	_svg2.selectAll("path.ref_mapping").data(_Chunk_ref_intervals).enter()  // 染色体下面的 三角形 path
...
function draw_chunk_variants() {
	...
	if (_Chunk_alignments.length > 0) {
		...
		if (_Bedpe.length > 0) {
			...
			_svg2.selectAll("path.bedpe_variants").data(variants_in_view).enter()  // 箭头 path
				.append("path")
...
function draw_chunk_alignments() {
	...
	var alignment_groups = _svg2.selectAll("g.alignment_groups").data(chunks).enter()
		...
		.on("click",function(d) {
			// _svg2.select("path#guide_line").attr("d", "M " + _positions.multiread.ref_intervals.x + ' ' + d.read_y + " L " + (_positions.multiread.ref_intervals.x+_positions.multiread.ref_intervals.width) +
			// ' ' + d.read_y).style("visibility", "visible")

			_svg2.select("rect#guide_line").attr("x",  _positions.multiread.ref_intervals.x).attr("y", d.read_y - 3).attr("width", _positions.multiread.ref_intervals.width).attr("height", 6).style("visibility", "visible")

			return new_read_selected(d.index);});
	...
	if (_settings.show_indels_as == "none") {
		...
		function chunk_alignment_path_generator(d) {
			var previous_x = _scales.chunk_ref_interval_scale(map_chunk_ref_interval(d.r,d.path[0].R)); // 起点
		...
		// Draw paths to allow indels   // 画 reads 的
		...
		if (_settings.show_indels_as != "none" && _settings.show_indels_as != "gaps") {
			...
			if (_settings.show_indels_as == "thin" || _settings.show_indels_as == "numbers") {
				...
				insertion_groups.append("circle")    // insertion circle
...
function draw_singleread_header() {
	...
	// Whole reference chromosomes for the relevant references:    // 下面图的染色体 rect
...
function draw_ribbons() {
	...
	// Draw alignments
	_svg.selectAll("path.alignment").data(_Alignments).enter()   // 下面部分的面积连线
		.append("path")
			.filter(function(d) {return d.mq >= _settings.min_mapping_quality && d.aligned_length >= _settings.min_align_length})
				.attr("class","alignment")
